...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const resultsRes = await fetch(`${API_URL}/api/results`, {
        credentials: "include",
      });
  ...
  const handleResultSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({
          type: "success",
          text: "Exam result published successfully!",
        });
        setResultForm({
          studentRoll: "",
          examName: "",
        ...
        setMessage({
          type: "error",
          text: data.error || "Failed to publish result",
        });
      }
    } catch (err) {
  ...
  const handleNoticeSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessage({
          type: "error",
          text: data.error || "Failed to publish notice",
        });
      }
    } catch (err) {
      setMessage({ type: "error", text: "Server connection error!" });
  ...
  const handleToggleBlock = async (id, currentStatus) => {
    ...
    try {
      ...
      if (data.success) {
        setStudents(
          students.map((s) =>
            s._id === id ? { ...s, isBlocked: newStatus } : s
          )
        );
        setAdmins(
          admins.map((a) =>
            a._id === id ? { ...a, isBlocked: newStatus } : a
          )
        );
      }
    } catch (err) {
  ...
  const handleToggleRole = async (id, currentRole) => {
    const newRole =
      currentRole.toLowerCase() === "admin" ? "student" : "admin";
    const confirmMsg =
      newRole === "admin"
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessage({
          type: "success",
          text: `User role successfully changed to ${newRole}!`,
        });
      } else {
        setMessage({
          type: "error",
  ...
  const handleUpdateResult = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/results/${editingResult._id}`, {
        ...
        credentials: "include",
        body: JSON.stringify({
          studentRoll: editingResult.studentRoll,
          examName: editingResult.examName,
          cqMarks: editingResult.cqMarks,
          mcqMarks: editingResult.mcqMarks,
        }),
      });
      const data = await res.json();
  ...
  const countClass11 = students.filter(
    (s) =>
      (s.studentClass ? String(s.studentClass).trim() : "11") === "11"
  ).length;
  const countClass12 = students.filter(
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900 p-4 sm:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="border-b border-slate-200/80 bg-white/90 backdrop-blur-md shadow-sm rounded-3xl p-6 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-400">
              Manage results, notices, students, and admins efficiently.
            </p>
          </div>
          ...
          <div className="flex flex-wrap bg-zinc-950 p-1.5 rounded-2xl border border-zinc-800 gap-1.5">
            ...
            >
              <FileText className="w-4 h-4" /> Results ({results.length})
            </button>
        ...
        <div className="border-b border-slate-200/80 bg-white/90 backdrop-blur-md shadow-smrounded-3xl p-6 sm:p-8 shadow-2xl">
       
          {activeTab === "result" && (
            <form onSubmit={handleResultSubmit} className="space-y-5">
              <h2 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2 text-indigo-600 border-b border-zinc-800 pb-3">
                <FileText className="w-4 h-4 text-indigo-600" /> Publish Result by
                Student Roll
              </h2>
          ...
          {activeTab === "results" && (
            <div className="space-y-6">
              <h2 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2 text-indigo-500 border-b border-zinc-800 pb-3">
                <FileText className="w-4 h-4 text-indigo-600" /> All Results (
                {results.length})
              </h2>

              {results.length === 0 ? (
                <p className="text-xs text-indigo-500 text-center py-8 italic">
                      ...
                      {editingResult?._id === result._id ? (
                        ...
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                              <label className="block text-xs text-indigo-500 mb-1">
                                Student Roll
                              </label>
                              <input
                                type="text"
                                value={editingResult.studentRoll}
                                onChange={(e) =>
                                  setEditingResult({
                                    ...editingResult,
                                    studentRoll: e.target.value,
                                ...
                                className="w-full text-indigo-500 border border-zinc-700 rounded-xl px-3 py-2 text-xs"
                                required
                              />
                            </div>
                            <div>
                          ...
                          <div className="flex gap-2 pt-2">
                            <button
                              type="submit"
                              disabled={loading}
                              className="px-4 py-2 bg-zinc-100 text-zinc-900 rounded-xl text-xs font-semibold"
                            >
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          <div className="space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-[10px] text-indigo-500 border border-zinc-700 px-2 py-0.5 rounded-lg">
                            ...
                            <p className="text-xs text-zinc-400">
                              CQ: {result.cqMarks} | MCQ: {result.mcqMarks} |
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-zinc-900 border border-zinc-800 text-zinc-300 hover:bg-zinc-800"
                            >
                              Edit
                            </button>
                            <button
                              onClick={() => handleDeleteResult(result._id)}
                              className="p-2 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-red-400 hover:border-zinc-700"
                              title="Delete Result"
                            >
          ...
          {activeTab === "notice" && (
            <div className="space-y-8">
              <form
                ...
                className="space-y-5 border-b border-zinc-800 pb-8"
              >
                <h2 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2 text-indigo-500">
                  <Bell className="w-4 h-4 text-indigo-500" /> Publish New Notice
                </h2>
                ...
                {notices.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {notices.map((notice) => (
                      <div
                        key={notice._id}
                        className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4 flex items-center justify-between gap-4"
                      >
          ...
          {activeTab === "students" && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-zinc-800 pb-4">
                <h2 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2 text-indigo-500">
                  <Users className="w-4 h-4 text-indigo-500" /> Enrolled Students
                ...
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                  {filteredStudents.map((student) => {
                    ...
                    return (
                      <div
                        key={student._id}
                        className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
                      >
          ...
          {activeTab === "admins" && (
            <div className="space-y-4">
              <h2 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2 text-indigo-500 border-b border-zinc-800 pb-3">
                <Shield className="w-4 h-4 text-indigo-500" /> Admin Directory (
                {admins.length})
              </h2>
                  ...
                  {admins.map((admin) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 w-full md:w-auto justify-end border-t md:border-t-0 pt-3 md:pt-0 border-zinc-800 flex-wrap">
                        ...
                        <button
                          onClick={() =>
                            handleToggleBlock(admin._id, admin.isBlocked)
                          }
                          className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1 transition-all ${
                            admin.isBlocked
                              ? "bg-zinc-900 border border-zinc-700 text-zinc-200 hover:bg-zinc-800"
