    ...
    async function fetchData() {
      try {
        ...
        if (studentData.success) {
          ...
          // একসঙ্গে সবার রেজাল্ট ফেচ করার জন্য Promise.all ব্যবহার করা হয়েছে (ফাস্ট করার জন্য)
          const resultPromises = onlyStudents.map(async (student) => {
            const roll = student.studentRoll ? String(student.studentRoll).trim() : "";
            if (!roll) return null;

            try {
              const res = await fetch(`${apiUrl}/api/results/roll/${roll}`);
              const resData = await res.json();
              if (resData.success) {
                return { roll, data: resData.data };
              }
            } catch (err) {
              console.error(`Failed to load result for roll ${roll}`, err);
            }
            return null;
          });
          ...
          for (const item of resultsArray) {
            if (item) {
              resultsMap[item.roll] = item.data;
            }
          }
