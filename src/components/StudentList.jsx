...
export default function StudentList() {
  ...
  const filteredStudents = students.filter((student) => {
    ...
    const passedAllA = resultsA.length > 0 && resultsA.every((res) => {
      const cq = Number(res.cqMarks || 0);
      const mcq = Number(res.mcqMarks || 0);
      return cq >= 15 && mcq >= 25;
    });

    const passedAllB = resultsB.length > 0 && resultsB.every((res) => {
      const cq = Number(res.cqMarks || 0);
      const mcq = Number(res.mcqMarks || 0);
      return cq >= 15 && mcq >= 25;
    });

    // First priority: students who passed ALL exams come first
      ...
      {filteredStudents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredStudents.map((student, index) => {
            ...
            // Check if this student passed ALL exams
            const passedAllExams = studentResults.length > 0 && studentResults.every((res) => {
              const cq = Number(res.cqMarks || 0);
              const mcq = Number(res.mcqMarks || 0);
              return cq >= 15 && mcq >= 25;
            });
